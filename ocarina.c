...
lval* lval_eval(lval* v){
  ...
  /*all other lval types remain same*/
  return v;
}
...
lval* lval_read_num(mpc_ast_t* t){
  ...
  return errno != ERANGE ? lval_num(x) : lval_err("invalid numbers");
}
...
int main(int argc, char** argv) {
  ...
  mpc_parser_t* Ocarina = mpc_new("ocarina");
  ...
  mpca_lang(MPC_LANG_DEFAULT,
    "                                                     \
      ...
      ocarina    : /^/ <expr>* /$/ ;           \
    ",
    Number, Symbol, Sexpr, Expr, Ocarina);

  puts("Ocarina Version 0.0.0.5");
  puts("Press Ctrl+c to Exit\n");

  while (1) {

    char* input = readline("Ocarina > ");
    add_history(input);

    mpc_result_t r;
    if (mpc_parse("<stdin>", input, Ocarina, &r)) {
  ...
  mpc_cleanup(5, Number, Symbol, Sexpr, Expr, Ocarina);
